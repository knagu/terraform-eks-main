...
data "aws_iam_policy_document" "eks_oidc_assume_role" {
  ...
  statement {
    ...
    condition {
      ...
      values = [
        "system:serviceaccount:${var.k8s_namespace}:${var.prefix}-${var.project}-${var.k8s_namespace}-aws-alb-ingress-controller"
      ]
    }
    principals {
      identifiers = [
...
resource "aws_iam_role" "this" {
  name        = "${var.prefix}-${var.project}-${var.k8s_namespace}--alb-ingress-controller"
...
resource "aws_iam_policy" "this" {
  name        = "${var.prefix}-${var.project}-${var.k8s_namespace}--alb-management"
...
resource "kubernetes_service_account" "this" {
  ...
  metadata {
    name        = "${var.prefix}-${var.project}-${var.k8s_namespace}-aws-alb-ingress-controller"
    ...
    labels = {
      "app.kubernetes.io/name"       = "${var.prefix}-${var.project}-${var.k8s_namespace}-aws-alb-ingress-controller"
              ...
              pod_affinity_term {
                label_selector {
                  match_expressions {
                    ...
                    operator = "In"
                    values   = ["${var.prefix}-${var.project}-${var.k8s_namespace}-aws-alb-ingress-controller"]
                  }
                }
